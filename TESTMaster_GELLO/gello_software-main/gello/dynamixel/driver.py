...
class DynamixelDriver(DynamixelDriverProtocol):
    ...
    def _initialize_hardware(self):
        ...
        # GELLO is a passive leader. Verify torque-off before starting the
        # background reader; do not continue with an unknown torque state.
    ...
    def set_torque_mode(self, enable: bool):
        ...
        with self._lock:
            for dxl_id in self._ids:
                current, read_result, read_error = self._packetHandler.read1ByteTxRx(
                    self._portHandler, dxl_id, ADDR_TORQUE_ENABLE
                )
                if read_result != COMM_SUCCESS or read_error != 0:
                    raise RuntimeError(
                        f"Failed to read torque mode for Dynamixel ID {dxl_id}: "
                        f"comm={read_result}, error={read_error}"
                    )
                if current == torque_value:
                    continue

                write_result = self._packetHandler.write1ByteTxOnly(
                    self._portHandler, dxl_id, ADDR_TORQUE_ENABLE, torque_value
                )
                if write_result != COMM_SUCCESS:
                    raise RuntimeError(
                        f"Failed to write torque mode for Dynamixel ID {dxl_id}: "
                        f"comm={write_result}"
                    )
                time.sleep(0.01)
                verified, read_result, read_error = self._packetHandler.read1ByteTxRx(
                    self._portHandler, dxl_id, ADDR_TORQUE_ENABLE
                )
                if (
                    read_result != COMM_SUCCESS
                    or read_error != 0
                    or verified != torque_value
                ):
                    raise RuntimeError(
                        f"Torque mode verification failed for Dynamixel ID {dxl_id}: "
                        f"expected={torque_value}, actual={verified}, "
                        f"comm={read_result}, error={read_error}"
                    )
    ...
    def _read_joint_states(self):
        ...
        while not self._stop_thread.is_set():
            # Seven servos at 57600 baud need a bounded bus interval; polling
            # faster than 20 Hz corrupts or times out the group-sync response.
            time.sleep(0.05)
            with self._lock:
