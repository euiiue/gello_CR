...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        window = create_operator_window(
            operator.backend,
            close_callback=operator.close,
            settings_factory=lambda: OperatorSettings(
                operator.runtime.store.path, type(operator.runtime.store)
            ),
        )
